...
# @shirlytaylor73/smart-search using a clean (non-mise-shim) python interpreter.
#
...
#
# Run after every `mise install` or `mise upgrade npm:@shirlytaylor73/smart-search`.
#
...
$pkgRoot = Join-Path $env:LOCALAPPDATA 'mise\installs\npm-shirlytaylor73-smart-search'
if (-not (Test-Path -LiteralPath $pkgRoot)) {
    Write-Error "smart-search mise install dir not found: $pkgRoot. Run 'mise install npm:@shirlytaylor73/smart-search@latest' first."
}
...
# npm extracts the package to node_modules/@shirlytaylor73/smart-search/, which is
# what npm/bin/smart-search.js treats as packageRoot (and where venvDir lives).
$packageDir = Join-Path $miseInstallDir.FullName 'node_modules\@shirlytaylor73\smart-search'
