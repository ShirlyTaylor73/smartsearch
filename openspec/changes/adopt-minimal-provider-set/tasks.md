...
- [ ] 1.1 重新核对本 change 的 proposal、design 与两份 spec，确认固定 provider 责任矩阵、breaking 边界和非目标一致
- [ ] 1.2 对照 Exa 官方 Search/Find Similar 文档记录当前请求字段、允许值、错误码和响应字段
- [ ] 1.3 对照 Firecrawl 官方 Scrape JSON/Markdown 与 Map 文档确定 `sitemap`、`location`、timeout 等最终 CLI 映射
- [ ] 1.4 通过 ZRead 官方文档或 MCP `tools/list` 固定 `search_doc`、`get_repo_structure`、`read_file` 的实际 input schema
- [ ] 1.5 在 `tests/` 先添加公开命令 → 唯一 provider/tool、禁止 fallback、删除旧参数/命令的失败测试
...
- [ ] 2.2 删除 `SMART_SEARCH_FALLBACK_MODE`、`SMART_SEARCH_OPERATION_CONFIG` provider 链与 `OPENAI_COMPATIBLE_FALLBACK_MODELS` 的执行语义
- [ ] 2.3 添加仅允许 operation timeout override 的 `SMART_SEARCH_OPERATION_TIMEOUTS` 配置契约与边界测试
- [ ] 2.4 将 operation profile 重构为固定 executor、必需配置、默认 timeout 与 normalizer descriptor
- [ ] 2.5 删除 provider candidate、feature negotiation、fallback runner 和 attempts 聚合路径
- [ ] 2.6 统一固定 executor 的配置、参数、认证、限流、超时、网络、解析与 provider 错误分类并补充脱敏测试
- [ ] 2.7 运行配置与 operation executor 聚焦测试，检查 diff 后提交确定性路由阶段 commit

## 3. Search 与 Docs 固定 Provider 实现

- [ ] 3.1 修改 `search answer` 只执行配置选中的 Grok transport，删除公开 `--stream/--no-stream` 和跨 transport fallback
- [ ] 3.2 修改 `search sources` 只调用 Exa Search，并逐项验证 mode、时间、domain、category、text、highlights 与 limit payload
- [ ] 3.3 修改 `search similar` 只调用 Exa Find Similar，并保留规范化 results/sources 输出
...
- [ ] 3.7 为 ZRead `search_doc.language` 实现经官方 schema 验证的语言推导/默认策略及测试
...
- [ ] 4.4 实现 Firecrawl Map 请求与 `--search`、`--sitemap`、subdomain、query、cache、limit、timeout、location 参数映射
...
- [ ] 5.4 删除 provider 专用隐藏 CLI 命令与 alias 转发，确保旧命令不会被不同 provider 执行
...
- [ ] 6.4 更新 `diagnose search|docs|fetch|map`，移除 candidates/single-provider/fallback 状态并检查固定 provider/tool
...
- [ ] 7.3 编写从多 provider/fallback、旧 provider 命令、ZRead `--ref`、Tavily Map 参数迁移到新版契约的对照表
...
- [ ] 8.5 在凭据可用时分别实际测试 Grok、Exa、Context7、ZRead、Firecrawl；如未运行必须明确记录原因，不得伪装通过
