...
- 明确唯一职责：`search answer` → Grok，`search sources|similar` → Exa，`docs resolve` 与普通 `docs search` → Context7，仓库定向 `docs search --source owner/repo` 及 `docs tree|read` → Zhipu MCP ZRead，`fetch content|extract` 与 `map site` → Firecrawl。
...
- Firecrawl adapter 需要覆盖 Scrape、JSON Schema extraction 与 Map；Exa adapter 需要完整承接 `search sources` 的公开过滤参数。
