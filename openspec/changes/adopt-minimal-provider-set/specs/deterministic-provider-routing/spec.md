...
系统 SHALL 使用不可由运行时配置重排的固定责任映射：`search.answer` → Grok，`search.sources` → Exa，`docs.resolve` → Context7，普通 `docs.search` → Context7，仓库 `docs.search` 与 `docs.tree|read` → Zhipu MCP ZRead，`fetch.content|extract` 与 `map.site` → Firecrawl；系统 SHALL NOT 注册 `search.similar` executor。
...
#### Scenario: 请求已删除的 Similar operation
- **WHEN** 服务层或 CLI 请求 `search.similar`
- **THEN** 系统将其视为不存在的 operation
- **AND** 不调用 deprecated Exa `findSimilar`
...
### Requirement: Exa Search Type 仅由配置控制
配置系统 SHALL 允许 `EXA_SEARCH_TYPE` 使用 Exa 当前原生枚举 `instant|fast|auto|deep-lite|deep|deep-reasoning`，默认 SHALL 为 `auto`，并 SHALL NOT 通过 Agent CLI 暴露 semantic/keyword 模式选择。

#### Scenario: 未配置 Exa Search Type
- **WHEN** 维护者未设置 `EXA_SEARCH_TYPE`
- **THEN** `search.sources` 请求使用 `type=auto`
- **AND** Agent 调用不需要提供 mode

#### Scenario: 配置无效 Exa Search Type
- **WHEN** `EXA_SEARCH_TYPE` 为 `neural`、`keyword` 或其他非官方枚举
- **THEN** 配置系统返回明确的配置参数错误
- **AND** 不向 Exa 发出请求
...
#### Scenario: 诊断 search capability
- **WHEN** 维护者运行 `smart-search diagnose search`
- **THEN** 系统只检查 `search.answer` 与 `search.sources`
- **AND** 不展示已删除的 `search.similar`
